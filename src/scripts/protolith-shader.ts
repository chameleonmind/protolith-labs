/**
 * ProtolithShader - WebGL morphing shape animation
 */
...
const fragSrc = `
  ...
  void main(){
    ...
    vec3 bg = vec3(0.9608, 0.9686, 0.9804);
...
export function initShader(canvas: HTMLCanvasElement, config: ShaderConfig): void {
  ...
  try {
    ...
    const colorSegmentSec = 1.6;
    const spinTurnsPerMorph = 1.5;
...
// Auto-initialize all shader canvases on page load
export function initAllShaders(): void {
  const canvases = document.querySelectorAll<HTMLCanvasElement>('[data-protolith-shader]');

  canvases.forEach((canvas) => {
    const paletteAttr = canvas.dataset.palette;
    const palette = paletteAttr
      ? JSON.parse(paletteAttr)
      : ["#ff5641", "#cf0177", "#7f38dc"];

    initShader(canvas, { palette });
  });
}
